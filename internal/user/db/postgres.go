...
func NewUserStorage(storage *pgx.Conn, requestTimeout int) user.Storage {
	return &db{
...
func (d *db) FindByEmail(email string) (*user.User, error) {
	query := fmt.Sprintf(`
...
func (d *db) FindByUsername(username string) (*user.User, error) {
	query := fmt.Sprintf(`
	SELECT id, username, email, password, verified, address, phone_number, TO_CHAR(registered_at, 'DD-MM-YYYY')
	FROM %s 
...
func (d *db) FindById(id int64) (*user.User, error) {
	query := fmt.Sprintf(`
	SELECT id, username, email, password, verified, address, phone_number, TO_CHAR(registered_at, 'DD-MM-YYYY')
	FROM %s 
...
func (d *db) Update(user *user.User) error {
	query := fmt.Sprintf(`
	UPDATE %s
	SET username=$1, email=$2, password=$3, verified=$4, address=$5, phone_number=$6
...
func (d *db) UpdatePartially(user *user.UpdateUserPartiallyDTO) error {
